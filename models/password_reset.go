...
func (service *PasswordResetService) Create(email string) (*PasswordReset, error) {
	// Verify there is a valid email address for user
	...
	err := row.Scan(&userID)
	if err != nil {
		return nil, fmt.Errorf("PasswordReset.Create: %w", err)
	}

	// Create a new PasswordReset struct
	...
	// Insert PasswordReset into the DB
	row = service.DB.QueryRow(`
...
func (service *PasswordResetService) Consume(token string) (*User, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("PasswordReset.Consume: %w", err)
	}

	// Validate that the PasswordReset is still valid
	if time.Now().After(pwReset.ExpiresAt) {
		return nil, fmt.Errorf("PasswordReset.Consume: %v token is expired", token)
	}

	// Delete the PasswordReset from the database to consume it
...
func (service *PasswordResetService) delete(id int) error {
	_, err := service.DB.Exec(`
		...
		where id = $1`, id)
	if err != nil {
		return fmt.Errorf("PasswordResetService.delete: %w", err)
	}
	return nil
}
